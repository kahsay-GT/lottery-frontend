      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={s.key} style={{ display: 'flex', alignItems: 'center' }}>
            ...
            {i < steps.length - 1 && (
              <div style={{ width: 36, height: 2, margin: '0 4px', marginBottom: 18, borderRadius: 99, background: done ? '#10b981' : 'rgba(255,255,255,0.08)', transition: 'background 0.4s' }} />
            )}
          </div>
        )
      })}
...
export function LotteryDetail() {
  ...
  const { data: lot, isLoading } = useQuery({
    queryKey: ['public-lot', slug],
    queryFn: () => lotteriesApi.publicGet(slug!).then(r => r.data?.data),
    enabled: !!slug,
  })
  ...
  // Reset selection when quantity changes
  ...
  const onSubmit = async (d: F) => {
    ...
    try {
      ...
      const resId = res.data?.data?.id ?? res.data?.id
  ...
  const onUpload = async () => {
    ...
    } catch (e: any) { toast.error(e?.response?.data?.message ?? e?.message ?? t('lotteryDetail', 'uploadFailed')) }
  ...
  if (!lot) return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center text-center">
      <div>
        ...
        <h2 className="text-2xl font-bold text-white">{t('lotteryDetail', 'lotteryNotFound')}</h2>
        <Link to="/lotteries" className="text-primary-400 hover:underline mt-2 inline-block">← Browse</Link>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="lot-detail-layout">
          ...
          <div style={{ position: 'sticky', top: 52, height: 'calc(100vh - 52px)', overflow: 'hidden', background: '#08090f' }}>
            ...
            {sliderImages.length === 0 && (
              <>
                ...
                <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 260, height: 260, background: 'radial-gradient(circle,rgba(99,102,241,0.3),transparent 70%)', zIndex: 1 }} />
              </>
            )}
            {/* Overlay: name + progress */}
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 6, padding: '48px 24px 24px', background: 'linear-gradient(to top,rgba(3,4,9,0.93) 0%,transparent 100%)' }}>
              <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, marginBottom: 6 }}>
                <h1 style={{ fontSize: 'clamp(16px,2vw,24px)', fontWeight: 900, color: '#fff', letterSpacing: '-0.02em', margin: 0, lineHeight: 1.2 }}>
                  {lot.name as string}
                </h1>
                <StatusBadge status={lot.status as string} />
              </div>
              {lot.description && (
                <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5, margin: '0 0 8px' }}>{lot.description as string}</p>
              )}
              {lot.drawDate && (
                <p style={{ fontSize: 11.5, color: '#fb923c', margin: '0 0 10px', fontWeight: 600 }}>
                  {fmtDate(lot.drawDate as string)}{dl > 0 ? ` · ${dl}d left` : ''}
                </p>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5, fontSize: 11 }}>
                <span style={{ color: barPct >= 80 ? '#fbbf24' : 'rgba(255,255,255,0.45)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 3 }}>
                  {barPct >= 80 && <Zap style={{ width: 10, height: 10, color: '#fbbf24' }} />}{pctStr} {t('lotteryDetail', 'ticketsSold')}
                </span>
                <span style={{ color: 'rgba(255,255,255,0.3)' }}>{sold.toLocaleString()} / {totalTix.toLocaleString()}</span>
              </div>
              <div style={{ height: 5, borderRadius: 99, background: 'rgba(255,255,255,0.12)', overflow: 'hidden', width: '100%' }}>
                <div style={{
                  height: 5, width: `${Math.max(barPct > 0 ? 2 : 0, barPct)}%`, borderRadius: 99,
        ...
        <div className="lot-detail-mobile-hero" style={{ position: 'relative', height: sliderImages.length > 0 ? 'clamp(200px,42vw,340px)' : 160, background: '#0d0e18', overflow: 'hidden' }}>
          <DetailSlider images={sliderImages} banner={lot.banner as string | null} alt={lot.name as string} />
          {sliderImages.length === 0 && (
            <>
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg,#1e1b4b,#312e81,#4c1d95)', zIndex: 0 }} />
              <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 200, height: 200, background: 'radial-gradient(circle,rgba(99,102,241,0.3),transparent 70%)', zIndex: 1 }} />
            </>
          )}
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 80, background: 'linear-gradient(to top,#030409,transparent)', zIndex: 3 }} />
        ...
        <div className="lot-detail-content">
          <div style={{ maxWidth: 600, margin: '0 auto', padding: '0 16px 80px' }}>
            ...
            {/* Mobile-only: title + date + progress (hidden on desktop where overlay shows it) */}
            <div className="lot-detail-mobile-meta" style={{ paddingTop: 20, marginBottom: 20 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: lot.description ? 6 : 14 }}>
                <h1 style={{ fontSize: 'clamp(20px,5vw,30px)', fontWeight: 900, color: '#fff', letterSpacing: '-0.02em', margin: 0, lineHeight: 1.2 }}>
                  {lot.name as string}
                </h1>
                {lot.drawDate && (
              ...
              {lot.description && <p style={{ fontSize: 14, color: '#9ca3af', lineHeight: 1.6, margin: '0 0 14px' }}>{lot.description as string}</p>}
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 12 }}>
                <span style={{ color: barPct >= 80 ? '#fbbf24' : '#9ca3af', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
                  {barPct >= 80 && <Zap style={{ width: 11, height: 11, color: '#fbbf24' }} />}{pctStr} {t('lotteryDetail', 'ticketsSold')}
                </span>
              ...
              <div style={{ height: 8, borderRadius: 99, background: 'rgba(255,255,255,0.08)', overflow: 'hidden', width: '100%' }}>
                <div style={{
                  height: 8, width: `${Math.max(barPct > 0 ? 2 : 0, barPct)}%`, borderRadius: 99,
                  background: barPct >= 80 ? 'linear-gradient(90deg,#f59e0b,#ef4444)' : 'linear-gradient(90deg,#4f46e5,#8b5cf6)',
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, overflow: 'hidden', marginBottom: 16 }}>
                  ...
                  {prizes.length > 0 ? prizes.map((p, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 8 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p className="prize-title" style={{ fontSize: 13, fontWeight: 600, margin: '0 0 2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.title as string}</p>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 14, borderRadius: 12, background: 'rgba(234,179,8,0.07)', border: '1px solid rgba(234,179,8,0.2)', marginBottom: 8 }}>
                      <Trophy style={{ width: 28, height: 28, color: '#fbbf24', flexShrink: 0 }} />
                      <div>
              ...
              {step === 'info' && (
                <div style={{ padding: '0 20px 24px' }}>
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    <div className="buy-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      <div>
                    ...
                    <div>
                      ...
                      <input className="input-dark" type="number" min={1} max={50}
                        {...register('quantity', { onChange: handleQtyChange })} />
                    ...
                    <div style={{ borderRadius: 16, padding: '16px 18px', background: 'linear-gradient(135deg,rgba(79,70,229,0.2),rgba(124,58,237,0.15))', border: '1px solid rgba(99,102,241,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button type="button" onClick={() => setStep('prizes')}
                        ...
                        <ArrowLeft style={{ width: 16, height: 16, display: 'block' }} />{t('lotteryDetail', 'back')}
                      </button>
                    ...
                    <div style={{ padding: '12px 14px', borderRadius: 12, background: 'rgba(99,102,241,0.07)', border: '1px solid rgba(99,102,241,0.2)' }}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        {confirmedNumbers.map(n => (
                          <span key={n} style={{ fontSize: 13, fontFamily: 'monospace', fontWeight: 800, padding: '4px 10px', borderRadius: 8, background: 'rgba(99,102,241,0.25)', color: '#a5b4fc', border: '1px solid rgba(99,102,241,0.4)' }}>#{n}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }} className="payment-amounts">
                    ...
                    {primaryBank && (
                      <div style={{ borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', padding: '14px 16px' }}>
                        <p style={{ fontSize: 10, fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 8px' }}>{t('lotteryDetail', 'paymentDetails')}</p>
                        <p style={{ fontSize: 12, color: '#9ca3af', margin: '0 0 2px' }}>{bankName}</p>
              ...
              {step === 'done' && (
                <div style={{ overflow: 'hidden' }}>
                  <div style={{ background: 'linear-gradient(135deg,rgba(16,185,129,0.15),rgba(5,150,105,0.1))', borderBottom: '1px solid rgba(52,211,153,0.15)', padding: '28px 20px', textAlign: 'center' }}>
                    <div style={{ width: 68, height: 68, borderRadius: '50%', background: 'rgba(52,211,153,0.15)', border: '2px solid rgba(52,211,153,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
                      <CheckCircle2 style={{ width: 38, height: 38, color: '#34d399', display: 'block' }} />
                    </div>
                    <h2 style={{ fontSize: 22, fontWeight: 900, color: '#fff', margin: '0 0 4px' }}>{t('lotteryDetail', 'allSet')} 🎉</h2>
                    <p style={{ fontSize: 13, color: '#9ca3af', margin: 0 }}>{t('lotteryDetail', 'allSetSub')}</p>
                  </div>
                  ...
                  <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
                    {confirmedNumbers.length > 0 && (
                      <div style={{ borderRadius: 14, background: 'rgba(16,185,129,0.07)', border: '1px solid rgba(52,211,153,0.2)', padding: '14px 16px' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                          {confirmedNumbers.map(n => (
                            <div key={n} style={{ padding: '8px 14px', borderRadius: 10, background: 'linear-gradient(135deg,rgba(52,211,153,0.15),rgba(16,185,129,0.1))', border: '1px solid rgba(52,211,153,0.3)', display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <div style={{ borderRadius: 14, background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.2)', padding: '14px 16px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                        ...
                        <button onClick={copyRef}
                          style={{ padding: '7px 12px', borderRadius: 10, background: copied ? 'rgba(52,211,153,0.12)' : 'rgba(255,255,255,0.06)', border: `1px solid ${copied ? 'rgba(52,211,153,0.3)' : 'rgba(255,255,255,0.1)'}`, cursor: 'pointer', color: copied ? '#34d399' : '#9ca3af', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0, whiteSpace: 'nowrap' }}>
            ...
            {lot.status === 'COMPLETED' && (drawData?.winners as unknown[])?.length > 0 && (
              <div style={{ padding: '0 0 48px' }}>
                <h2 style={{ fontSize: 20, fontWeight: 900, color: '#fff', textAlign: 'center', margin: '0 0 16px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                  <Trophy style={{ width: 22, height: 22, color: '#fbbf24', display: 'block' }} />{t('lotteryDetail', 'winners')}
                </h2>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {(drawData.winners as Record<string, unknown>[]).map((w, i) => {
                    const medals = [
                      { e: '🥇', color: '#fbbf24', bg: 'rgba(251,191,36,0.08)', border: 'rgba(251,191,36,0.2)' },
                      { e: '🥈', color: '#9ca3af', bg: 'rgba(156,163,175,0.08)', border: 'rgba(156,163,175,0.2)' },
                      { e: '🥉', color: '#cd7c2f', bg: 'rgba(205,124,47,0.08)', border: 'rgba(205,124,47,0.2)' },
                    ]
                    const m = medals[i] ?? { e: '🏅', color: '#818cf8', bg: 'rgba(99,102,241,0.08)', border: 'rgba(99,102,241,0.2)' }
                    ...
                    return (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', borderRadius: 16, background: m.bg, border: `1px solid ${m.border}` }}>
                        <span style={{ fontSize: 28, flexShrink: 0 }}>{m.e}</span>
                        <div style={{ flex: 1, minWidth: 0 }}>
        ...
        </div>{/* end lot-detail-content */}

      </div>{/* end lot-detail-layout */}
      ...
      <style>{`
        ...
        .lot-detail-layout      { display: block; }
        .lot-detail-image       { display: none; }
        .lot-detail-mobile-hero { display: block; }
        .lot-detail-mobile-meta { display: block; }
        .lot-detail-content     { display: block; }
        ...
        .lot-detail-image {
          ...
          .lot-detail-mobile-hero { display: none !important; }
          .lot-detail-mobile-meta { display: none !important; }
          .lot-detail-content {
            grid-column: 2;
            overflow-y: auto;
            padding-top: 28px;
